...
Provided our example is a purely gradient system, we expect our approximation error to be zero everywhere.
...
Being this a purely non-gradient system we expect our pseudopotential to not be trustworthy. By calculating the error we can see that actually that's the case:
...
Even for highly non-gradient systems, our method will compute some pseudopotential.
...
By plotting the estimated error, we can see that it is high almost everywhere. This means that our algorithm noticed that the system is highly non-gradient, and thus, the previously computed pseudopotential is of very limited use.
